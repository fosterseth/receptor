...
func checkReload() error {
	...
	// Determine which items from the old config are no longer present, or modified
...
func (c *reloadCommand) ControlFunc(ctx context.Context, nc *netceptor.Netceptor, cfo ControlFuncOperations) (map[string]interface{}, error) {
	...
	if !backendModified && !loglevelModified {
	...
	// determine if log-level is defined in the new config
